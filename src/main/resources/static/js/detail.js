...
function loadJsonp3(url,titleName,ct,noHead){
	...
	$.ajax({
		...
		success:function(result){
			if(typeof(result.data.content) != "undefined" && result.data.content=="加载中失败！"){
				top.location=ct+"/app/menu/interface/getData?url=http://www.sdggzy.cn/data/main.php?id="+id.replace("_","-")+"&title="+titleName+"&adapt=1&noHead="+noHead;         
			}else{
				showData(result.data,true);
			}
...
function loadXml(url,pageIndex,flag){
	...
	$.ajax({
		...
		success:function(data){
			alert("Success");
			alert($(data).find("Response").find("Head").find("message").text());
			return data;
		},
		error:function(){
			alert("出错啦...");
