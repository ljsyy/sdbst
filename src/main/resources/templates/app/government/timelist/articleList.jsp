...
function article(){
	$.ajax({ 
		url: "${ct}/f/cms/articleApi/articleMap",
		data: "pageIndex="+pageNum+"&pageSize=20&infoType=${param.id}",
		type:"POST",
		dataType:"json",
		async:false,
		success:function(result){
			...
			for(var i in obj){
				...
				if(obj[i].link.indexOf("http")==0){ 			// 跳转的是 外网网址
					...
					href="${ct}/app/menu/interface/getData?url="+obj[i].link+"&title=新闻&noHead=true&share=%20&articleId="+obj[i].id;
				}else{											// 跳转的是内部sid详情链接(用于认识顺德-旅游信息)
					href="${ct}"+obj[i].link+"&noHead=true";
				}
				$(".main").append(
