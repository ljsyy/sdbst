 ...
 * http://www.jeasyui.com/license_commercial.php 二次开发 ____′↘夏悸
...
(function($) {
	...
	script.src = basePath() + "/js/app/calendar/jquery-1.8.3.min.js";
	document.getElementsByTagName('head')[0].appendChild(script);
	$.ajax({
		url : basePath() + "/calendar/getExamCalendar",
		type : "get",
		async : false,
		dataType : "json",
		success : function(result) {
			...
			var arr = new Array();
			for ( var d in data) {
				var examname = data[d].examname;
				var signuptime = data[d].signuptime;
				if (data[d].examtimetag != undefined) {
					var examtimetag = data[d].examtimetag.split(",");
					for ( var e in examtimetag) {
						arr.push(examtimetag[e] + " " + examname + "(报名时间:"
								+ signuptime + ")");
					}
				}
			}
	...
	var monthName = new Array("正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月",
			"九月", "十月", "冬月", "腊月");
	/***************************************************************************
	 * 日期计算
	 **************************************************************************/
	...
	function Calendar(y, m) {
		...
		for (var i = 0; i < this.length; i++) {
			...
			// 依节气调整二月分的年柱, 以立春为界
			/*
			 * //PM提出线上2月3日，初一不是庚寅年，应该是辛卯年。 by yuji
			 *
			 * 这里firstNode是指农历每月的节气所在的日期，用这个标志判断 农历每月起始日天干地支是错误的，应当用每月的农历初一所在日确定
		...
		$(target)
				...
				.wrapInner(
						'<div class="calendar-header">'
								+ '<a class="calendar-prevyear" style="transform: scale(1.5);-webikit-transform: scale(1.5);margin-right: 5px;"></a>'
								+ '&nbsp;'
								+ '<a class="calendar-year"></a>'
								+ '&nbsp;'
								+ '<a class="calendar-nextyear" style="transform: scale(1.5);-webikit-transform: scale(1.5);margin-left: 5px;"></a>'
								+ '&nbsp;'
								+ '<a class="calendar-prevmonth" style="transform: scale(1.5);-webikit-transform: scale(1.5);margin-right: 5px;"></a>'
								+ '&nbsp;'
								+ '<a class="calendar-month"></a>'
								+ '&nbsp;'
								+ '<a class="calendar-nextmonth" style="transform: scale(1.5);-webikit-transform: scale(1.5);margin-left: 5px;"></a>'
								+ '</div>'
	 ...
	 * created then create it.显示可以改变年或月的选择菜单,如果不创建菜单然后创建它。
	 */
	function showSelectMenus(target) {
		...
		if ($(target).find('.calendar-menu-month-inner').is(':empty')) {
			...
			var t = $('<table cellspacing="5" cellpadding="5" ></table>')
	...
	function show(target) {
		...
		if (opts.year > 1874 && opts.year < 1909)
			yDisplay = '光绪'
					+ (((opts.year - 1874) == 1) ? '元' : opts.year - 1874);
		if (opts.year > 1908 && opts.year < 1912)
			yDisplay = '宣统'
					+ (((opts.year - 1908) == 1) ? '元' : opts.year - 1908);

		if (opts.year > 1911)
			yDisplay = '建国'
					+ (((opts.year - 1949) == 1) ? '元' : opts.year - 1949);

		// $(target).find('.calendar-title span').html(opts.months[opts.month -
		// 1] + ' ' + opts.year + ' ' + yDisplay + '年 农历 ' + cyclical(opts.year
		// - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】');
		// 这是日历头部
		$(target).find('.calendar-year').html(opts.year + '年');
		$(target).find('.calendar-month').html(
				opts.months[opts.month - 1] + '月');
		$(target).find('.calendar-title span').html(
				yDisplay + '年 农历 ' + cyclical(opts.year - 1900 + 36) + '年 【'
						+ Animals[(opts.year - 4) % 12] + '年】')
				...
				if (opts.month == day[1]) {
					...
					if (opts.lunarDay) {
						...
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<span color="#E0366E">'
									+ info.solarTerms + '</span>';
						} else {
							dayHtml += '<span size="2" style="font-size:9pt">'
									+ cDay(info.lDay, info.lMonth) + '</span>';
						}
					}
				}
				var day = $(
						'<td class="calendar-day fullcalendar-day calendar-other-month"></td>')
						.data('info', info).attr('abbr',
								day[0] + '-' + day[1] + '-' + day[2]).html(
								dayHtml).appendTo(tr);
				...
				day
						.hover(
								function(e) {
									...
									if (inf) {
										var ct = '<font color="#ffffff" style="font-size:9pt;">'
												+ inf.sYear
												+ ' 年 '
												+ inf.sMonth
												+ ' 月 '
												...
												+ ' 日<br>星期'
												+ inf.week
												+ '<br><font color="white">农历 '
												+ monthName[inf.lMonth - 1]
												+ ' 月 '
												+ cDay(inf.lDay, inf.lMonth,
														true)
												+ ' 日</font><br><font color="yellow">'
												+ inf.cYear
												+ '年 '
												+ inf.cMonth
												+ '月 '
										...
										if (inf.lunarFestival) {
											detail
													.append('<div class="lunarFestival">'
															+ inf.lunarFestival
															+ '</div>');
										}
										if (inf.solarFestival) {
											detail
													.append('<div class="solarFestival">'
															+ inf.solarFestival
															+ '</div>');
										}
									} else {
		...
		if (opts.current) {
			t.find('.calendar-selected').removeClass('calendar-selected').css(
					'backgroundColor', '');
			...
			t.find('td[abbr="' + current + '"]').addClass('calendar-selected')
					.css('backgroundColor', 'rgb(129, 208, 242)');
		}
		...
		if (suIndex >= 7)
			suIndex -= 7;
		t.find('tr').find('td:eq(' + saIndex + ')').addClass(
				'calendar-saturday');
			...
			$(this).removeClass('calendar-hover');
				...
				function() {
					...
					t.find('.calendar-selected').removeClass(
							'calendar-selected').css('backgroundColor', '');
					$(this).addClass('calendar-selected').css(
							'backgroundColor', 'rgb(129, 208, 242)');

					// 获取到当前日期
					var parts = $(this).attr('abbr').split('-');

					// 动态添加div给予赋值
					window.parent.$("#iframe").append(
							'<div class="div_val">' + parts + '</div>');
					// 获取div的值
					var div_val = window.parent.$("#iframe").find(".div_val")
							.text();
					// 替换 所有，逗号
					var rep = div_val.replace(/\,/g, '-');
					// 判断div是否存在 存在就移除掉
					if (window.parent.$("#iframe").find(".div_val").hasClass(
							"div_val")) {
	...
	function calculatePos(target, currentTarget) {
		var w = $(this).width(), h = $(this).height(), css = {}
		...
		return this.each(function() {
			...
			if (state) {
				...
				state = $.data(this, 'fullCalendar', {
					options : $.extend({}, $.fn.fullCalendar.defaults,
							$.fn.calendar.defaults, $.fn.fullCalendar
	...
	$('head')
			.append(
					'<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
